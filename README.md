# C Basics :hammer:

> A linguagem C é uma das linguagens mais importantes do mundo da programação. Este repositório é dedicado ao estudo dessa linguagem, buscando mostrar o básico da linguagem e alguns exemplos.

## Sumário

- [**Introdução**](./content/intro)
  - [**Linux, WSL e Shell Script (Bash)**](./content/intro/linux_wsl_shell_script.md)
  - [**Algoritmos**](./content/intro/algorithms.md)
  - [**Introdução a Linguagem C**](./content/intro/c_lang.md)
- [**Variáveis**](./content/variables/variables.md)
- [**Operadores**](./content/operators/)
  - [**Aritméticos**](./content/operators/arithmetic_operators.md)
  - [**Atribuição**](./content/operators/attribution_operators.md)
  - [**Relacionais**](./content/operators/relational_operators.md)